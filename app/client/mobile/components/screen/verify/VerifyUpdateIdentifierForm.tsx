...
import { VerifyIdentifierUpdateFormFields } from "@/types/api/auth";
...
type VerifyUpdateIdentifierFormProps = {
  className?: string;
};

type VerifyFormFields = Array<string>;

type StepProps<T extends {}> = Pick<UseFormReturn<T>, "control" | "trigger" | "watch"> & {
...
export const VerifyUpdateIdentifierForm = ({
  ...
  type SchemaFormFields = InferType<typeof schema>;
  ...
  const steps: Step<SchemaFormFields>[] = useMemo(
...
const Step1: <T extends {}>(props: StepProps<T>) => ReactNode = ({
  type,
  identifier,
  control,
  trigger,
...
const Step2 = ({ control, trigger, errors, goToNextStep, type, watch }: StepProps) => {
  const [isLoading, setIsLoading] = useState(false);
...
const Step3 = (_: VerifyIdentifierUpdateStepProps) => {
