...
public class AdminGetNumberOfRecipesStatisticQueryHandler : IRequestHandler<AdminGetNumberOfRecipesStatisticQuery, Result<List<StatisticEntity>?>>
{
    ...
    public async Task<Result<List<StatisticEntity>?>> Handle(AdminGetNumberOfRecipesStatisticQuery request, CancellationToken cancellationToken)
    {
        var range = request.RangeType;
        var lang = request.Language;
        if (string.IsNullOrEmpty(range) || string.IsNullOrEmpty(lang))
        {
            return Result<List<StatisticEntity>?>.Failure(RecipeError.NullParameter, "Language or RangeType is null");
        }
        ...
        if (lang != "en" && lang != "vi")
        {
            return Result<List<StatisticEntity>?>.Failure(RecipeError.NullParameter, "Invalid language:" + lang);
        }
        ...
        switch (range.ToLower())
        {
            case "24h":
                ...
                return Result<List<StatisticEntity>?>.Success(GetHourlyStatistics(startTime, now));
            case "7d":
                startTime = now.AddDays(-7);
                return Result<List<StatisticEntity>?>.Success(GetDailyStatistics(startTime, now, lang));
            case "30d":
                startTime = now.AddDays(-30);
                return Result<List<StatisticEntity>?>.Success(GetDailyStatistics(startTime, now, lang, "dd/MM/yyyy"));
            case "3m":
                startTime = now.AddMonths(-3);
                return Result<List<StatisticEntity>?>.Success(GetThirtyDaysStatistics(startTime, now, lang));
            case "12m":
            case "24m":
                int months = int.Parse(range[..^1]);
                startTime = new DateTime(now.Year, now.Month, 1).AddMonths(-months + 1);
                return Result<List<StatisticEntity>?>.Success(GetMonthlyStatistics(startTime, now, lang));
            default:
                return Result<List<StatisticEntity>?>.Failure(RecipeError.NullParameter, "Invalid RangeType:" + range);
        }
    }
