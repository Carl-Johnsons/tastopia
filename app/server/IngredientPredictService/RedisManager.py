    ...
    def get_approx_prediction_from_cache(self, phash: str):
        ...
        for key_item in keys_str:
            ...
            if result < 5 and result > 0:
                logging.info(
                    f"cache hit for: {str(imagehash.hex_to_hash(raw_key))} - {str(imagehash.hex_to_hash(phash))} = {result}"
                )
                return self.get_prediction_from_cache(raw_key)
            else:
                logging.info(
                    f"cache miss, getting prediction for: {str(imagehash.hex_to_hash(raw_key))} - {str(imagehash.hex_to_hash(phash))} = {result}"
                )
